  ...
  return (
    <div className="w-screen h-screen bg-blue-200 flex justify-center p-6">
      <div className="w-[420px] space-y-6">
        <h1 className="text-3xl text-black font-bold text-center">Cadastro</h1>
        <form onSubmit={onSubmit} className="space-y-4 p-6 bg-white rounded-md shadow">
          <input required placeholder="Nome" value={name} onChange={e=>setName(e.target.value)} className="w-full px-3 py-2 border rounded" />
          ...
          <div className="flex gap-2">
            <button type="submit" className="bg-green-600 text-black px-4 py-2 rounded flex-1" disabled={loading}>{loading ? 'Criando...' : 'Cadastrar'}</button>
            <button type="button" onClick={() => navigate('/login')} className="bg-red-300 px-4 py-2 rounded">Voltar</button>
          </div>
        </form>
      </div>
    </div>
